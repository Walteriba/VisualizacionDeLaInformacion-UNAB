...
```{r ggplot-color}
ggplot(temperaturas, aes(x = day_of_year, y = temperature, color = location)) +
  geom_line()
```
...
```{r ggplot-color2}
ggplot(temperaturas, aes(x = day_of_year, y = location, color = temperature)) +
  geom_point(size = 10) # Un poco mas grande, asi se nota mejor el gradiente
```
...
```{r ggplot-fill}
ggplot(temperaturas, aes(x = day_of_year, y = temperature, color = location)) +
  geom_point(aes(color = location), shape = 21, size = 3) #puntos más grandes para ver mejor las diferencias
```
...
```{r ggplot-fill2}
ggplot(temperaturas, aes(x = day_of_year, y = temperature)) +
  geom_point(aes(fill = location), shape = 21, size = 3) #puntos más grandes para ver mejor las diferencias
```
...
```{r ggplot-params}
ggplot(temperaturas, aes(x = day_of_year, y = temperature)) +
  geom_point(aes(fill = location),color = "red", shape = 21, size = 3) #puntos más grandes para ver mejor las diferencias
```
...
```{r ggplot-params2}
ggplot(temperaturas, aes(x = day_of_year, y = temperature)) +
  geom_point(aes(color = location), fill = "red", shape = 21, size = 3) #puntos más grandes para ver mejor las diferencias
```
